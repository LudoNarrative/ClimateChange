...
def hashable(obj):
    if isinstance(obj, collections.Hashable):
        ...
        items = frozenset((k, hashable(v)) for k, v in sorted(obj.iteritems()))
    elif isinstance(obj, collections.Iterable):
        items = tuple(hashable(item) for item in sorted(obj))
    else:
...
if __name__ == '__main__':
    ...
    for output_ind, out in enumerate(outs[:]):
        ...
        def control_event_text(condition, by=False):
            out_string = ''
            if 'click' == condition['predicate']:
                out_string += 'click{} '.format("ing" if by else "") + sprites[prettify(condition['terms'][0])] + '\n'
            elif 'button' == condition['predicate']:
                verb = condition['terms'][1]['predicate'] + ("ing" if by else "")
        ...
        def preconditions_to_string(preconditions, by=False):
            out_string = ""
            for ind, precond in enumerate(preconditions):
                precond = precond['terms'][0]
                if 'overlaps' == precond['predicate']:
                    if 'true' == precond['terms'][2]['predicate']:
                        out_string += 'mak{} '.format("ing" if by else "e") + sprites[prettify(precond['terms'][1])] + ' touch ' + sprites[prettify(precond['terms'][0])] + '\n'
                    else:
                        out_string += '\tkeep{} '.format("ing" if by else "") + sprites[prettify(precond['terms'][1])] + ' away from ' + sprites[prettify(precond['terms'][0])] + '\n'
                elif 'control_event' == precond['predicate']:
                    out_string += '\t' + control_event_text(precond['terms'][0], by=by)
                elif 'compare' == precond['predicate']:
                    if 'le' == precond['terms'][0]['predicate']:
                        out_string += 'minimiz{} {}'.format("ing" if by else "e", sprites[prettify(precond['terms'][1])]) + '\n'
                    elif 'ge' == precond['terms'][0]['predicate']:
                        out_string += 'maximiz{} {}'.format("ing" if by else "e", sprites[prettify(precond['terms'][1])]) + '\n'
                else:
        ...
        colors = ['red', 'blue', 'green', 'orange', 'white', 'black', 'clear']
        for c in colors:
            sprites['{}'.format(c)] = '{}'.format(c)
            sprites['amount({})'.format(c)] = '{}'.format(c)
        out_string += '<ul>\n'
        ...
        for o in ['reading']:
            for oo in out[o]:
                for ooo in oo:
                    ...
                    print reading
                    # TODO: take the outcomes directly causing this goal and put them right here
                    # TODO: why do we not print anything for amount or distance?
                    if 'goal' in reading and 'amount' not in reading and 'distance' not in reading:
                        bystring = ""
                        for o2 in ['reading']:
                            for oo2 in out[o2]:
                                for ooo2 in oo2:
                                    reading2 = prettify(ooo2)
                                    if 'outcome_helps' in reading2 and ooo['terms'][1] == ooo2['terms'][1]['terms'][1]:
                                        outcome = hashable(ooo2['terms'][1]['terms'][0])
                                        shown_already.add(outcome)
                                        preconditions = outcome2precond[outcome]
                                        if bystring != "":
                                            bystring += " or "
                                        bystring += preconditions_to_string(preconditions, by=True)
                        if bystring != "":
                            bystring = " by " + bystring
                        out_string += "<li>" + prettify(ooo['terms'][0]['terms'][0]).title() + " " + sprites[prettify(ooo['terms'][1])] + bystring + '</li>\n'
        ...
        if will_dos:
            ...
            for outcome_ind, outcome in enumerate(will_dos):
                if outcome in shown_already:
                    continue
                out_string += '<li>\n'
                # TODO don't repeat instructions that appeared in GOAL
                out_string += preconditions_to_string(outcome2precond[outcome])
                out_string += '</li>\n'
            out_string += '</ul></li>\n'
        ...
        if avoids:
            ...
            for outcome_ind, outcome in enumerate(avoids):
                out_string += '<li>\n' + preconditions_to_string(outcome2precond[outcome], by=True) + "</li>\n"
            out_string += '</ul>\n'
        ...
        for oo in out['entity_movement']:
            for ooo in oo:
                ...
                if entity in player_controls:
                    ...
                    out_string += '<li>\t{} to make {} {}</li>\n'.format(control_event_text(player_controls[entity]), entity, action)
        # for oo in out['entity_movement']:
        #    for ooo in oo:
        #        pretty = prettify(ooo)
        #        ooo = ooo['terms'][0]
