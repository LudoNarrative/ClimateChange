...
var init = function(id, _State, _Display) {
	...
	game = new Phaser.Game(500, 550, Phaser.AUTO, 'gameContainer', { preload: preload, create: create }, false);

	var variables={'confidence':'5','optimism':'2','difficulty':'3'};
	var low=1;var r1=low;var e1;var e2; var gridLinesHorizontal; var gridLinesVertical; var grid; var gridIdx; var gridSize; 
	function preload(){game.load.image('e1','assets/sprites/glowing_unicorn.png');
	game.load.image('e2','assets/sprites/pineapple.png');};
	function create(){
		gridSize=30; 
	...
	function getRandomPoint(){
		var x=game.rnd.integerInRange(0,game.width-1);
		console.log(x);
		var y=game.rnd.integerInRange(0,game.height-1);console.log(y);
	...
	function initGrid(){
		...
		for(var i=0;i<gridLinesHorizontal;i++){
			for(var j=0;j<gridLinesVertical;j++){
				grid.push(new Phaser.Point(i*gridSize,j*gridSize));
			}
		}
		shuffle(grid);
		return grid;
