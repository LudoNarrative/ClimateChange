...
def parse_game(result):
    ...
    for initialization in result['initialize']:
        ...
        if 'set_value' == pred:
            settings.append(('initialize', prettify(children[0]),
                             [int(children[1]['terms'][0]['predicate'])],
                             'set'))
        if 'add' in pred:
            if prettify(children[0]) not in adds:
                adds[prettify(children[0])] = 0
            amount = int(children[1]['terms'][0]['predicate'])
            adds[prettify(children[0])] += amount
    for add in adds:
    ...
    for outcome in outcome_to_preconditions:
        ...
        for precondition in outcome_to_preconditions[outcome]:
            event = precondition['terms'][0]
            if event['predicate'] == 'control_event':
                inner_pred = event['terms'][0]['predicate']
                if inner_pred == 'button':
                    has_press = True
                if inner_pred == 'click':
                    has_click = True
