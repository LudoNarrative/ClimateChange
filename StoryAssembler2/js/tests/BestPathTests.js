...
define(["../Wishlist", "../ChunkLibrary", "../Request", "../State"], function(Wishlist, ChunkLibrary, Request, State) {
	...
	var run = function() {
		...
		test("bestPath", function( assert ) {
			...
			wl = Wishlist.create([{request: "x eq 1"}], State);
			ChunkLibrary.add([
				{ id: "alpha", content: "What do you choose?", choices: [Request.byId("Choice1"), Request.byCondition("z eq 5")] },
			...
			ChunkLibrary.add([
				...
				{ id: "ChoiceNode", content: "...", effects: ["set x true"], choices: [Request.byId("ResultNode")] },
				{ id: "ResultNode", choiceLabel: "...", content: "...", effects: ["set y true"] }
			]);
			wl.logOn();
			nextPath = wl.bestPath(ChunkLibrary);
			wl.logOff();
			assert.deepEqual(nextPath.route, ["ChoiceNode", "ResultNode"], "should maximize Wants through choice structures.");
		...
		test("allPaths", function( assert ) {
			...
			wl = Wishlist.create([{request: "x eq true"}], State);
			ChunkLibrary.add([
				{ id: "Node1", content: "...", choices: [Request.byId("Node2"), Request.byId("Node3")], effects: ["set x true"] },
